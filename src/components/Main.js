                            ...
                            <div>
                                <form onSubmit={
                                    (e)=>{
                                        ...
                                        this.props.commentToComment(budditId,{text:input.value,sub:[],author:this.props.match.params.login},nowKey);
                                        input.value='';
        ...
        <div>
            {
                ...
                <div className='buddit-body'  key={buddit.id} >
                   <div className='hold-buddit'>
                       <div className="c1">
                           <span className='pointable' onClick={()=>(this.props.upVote(buddit.id,this.props.match.params.login))}>&#x25B2;</span>
                           <p className='buddit-p'>{buddit.score}</p>
                           <span className='pointable' onClick={()=>(this.props.downVote(buddit.id,this.props.match.params.login))}>&#x25BC;</span>
                       </div>
                       <div className="c2">
                           {
                                buddit.image?
                                <img className='imag-stuff buddit-p' src={buddit.image} alt='no image'/>:
                                <p>no image</p>
                           }
                       ...
                       <div className="c3">
                           ...
                           <p className='buddit-p submitted'>submitted on {buddit.date} by <span className="highlight-username">{buddit.username}</span></p>
                       ...
                       <span className='to-bold text-btn pointable' onClick = {()=>{
                                        if(this.state.showCommentsFor==buddit.id){
                                            ...
                                            this.setState({showCommentsFor:buddit.id,
                                            commentsToAddTo:-1})
                                        }
                                    }
                                }
                            ...
                            <div>
                                <form onSubmit={
                                    (e)=>{
                                        ...
                                        this.props.addComment(buddit.id,{text:input.value,sub:[],author:this.props.match.params.login});
                                        input.value='';
                                        this.setState({
                                            showCommentsFor:buddit.id,
                                            commentsToAddTo:-1
            ...
            <p className='buddit-p'>You are logged in as <span className='to-bold  highlight-username'>{this.props.match.params.login}</span></p>
        </div>
    )
    }
}
